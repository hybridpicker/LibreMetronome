...
export default function usePolyrhythmLogic({
   ...
   * with enhanced accuracy and stability around common tempos
   */
  const tapTempo = useCallback(() => {
    ...
    // Limit to last 6 taps for better accuracy
    if (tapTimesRef.current.length > 6) {
      tapTimesRef.current.shift();
    }
    
    // Constants for 120 BPM stability
    const COMMON_BPM_STABILITY_THRESHOLD = 0.05; // 5% threshold for snap-to
    const COMMON_TEMPOS = [60, 90, 100, 120, 140, 160, 180]; // Common tempos
    ...
    if (tapTimesRef.current.length >= MIN_TAPS_REQUIRED) {
      ...
      // Sort intervals and remove outliers if we have enough data
      if (intervals.length >= 3) {
        const sortedIntervals = [...intervals].sort((a, b) => a - b);
        // Only remove outliers if we have enough samples
        if (sortedIntervals.length > 4) {
          // Remove fastest and slowest to reduce anomalies
          sortedIntervals.shift();
          sortedIntervals.pop();
          
          // Use the filtered intervals
          const filteredAvg = sortedIntervals.reduce((sum, val) => sum + val, 0) / sortedIntervals.length;
          console.log(`[POLYRHYTHM] Filtered average interval: ${Math.round(filteredAvg)}ms`);
          
          // Convert to BPM with higher precision
          const calculatedTempo = 60000 / filteredAvg;
          ...
          for (const commonTempo of COMMON_TEMPOS) {
            ...
            if (percentDifference < COMMON_BPM_STABILITY_THRESHOLD) {
              console.log(`[POLYRHYTHM] Snapping from ${calculatedTempo.toFixed(2)} to ${commonTempo} BPM (common tempo)`);
          ...
          // Only set tempo after we have enough taps for accurate measurement
          if (tapTimesRef.current.length >= REQUIRED_TAPS) {
            console.log(`[POLYRHYTHM] Setting stable tempo to ${newTempo} BPM`);
          ...
          // Simple average for fewer samples
        ...
        // Simple average for fewer intervals
        const avgInterval = intervals.reduce((sum, val) => sum + val, 0) / intervals.length;
        console.log(`[POLYRHYTHM] Average interval: ${Math.round(avgInterval)}ms`);
